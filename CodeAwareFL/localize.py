...
class RuntimeContext(object):
    ...
    def __init__(self):
        ...
        self.train_dir = config_parser.get(file_section, "train_dir")
        ...
        self.IA_test_data_file = config_parser.get(file_section, "IA_test_data_file")
...
def convert_examples_to_features_IA(item):
    # source
    url1, label, tokenizer, args, cache, url_to_code = item
    parser = parsers['c']
    ...
    for url in [url1]:
        if url not in cache:
            func = url_to_code[url]
            # extract data flow
            code_tokens_orig, dfg_all, chain_num_list = extract_dataflow_IA(url,func, parser, 'c')
            ...
            for dfg in dfg_all:
                ...
                dfg_to_code = [(x[0]+length, x[1]+length) for x in dfg_to_code]
                cache[url] = source_tokens, source_ids, position_idx, dfg_to_code, dfg_to_dfg
                InputFeatures_result=InputFeatures_IA(source_tokens, source_ids, position_idx, dfg_to_code, dfg_to_dfg,label, url1, chain_num_list,code_tokens_orig,dfg_all[0],func)
    
    return InputFeatures_result


class TextDataset_IA(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        self.examples_all = []
        ...
        # convert example to input features
        self.examples_all = [convert_examples_to_features_IA(
            x) for x in tqdm(data, total=len(data))]
...
def test_IA(args, model, tokenizer, best_threshold=0):
    # build dataloader
    eval_dataset = TextDataset_IA(tokenizer, args, file_path=args.IA_test_data_file)
    eval_sampler = SequentialSampler(eval_dataset)
    ...
    for item_eval_dataset in eval_dataset:
        ...
        result_source = []
        f5 = open('dataset/sliceResult.txt','r')
        f6 = open('dataset/covMatrix_new.txt','r')
        for line_num in f5:
            line_num = line_num.split()
        for line_cov in f6:
            line_cov = line_cov.split()
        for item in final_result:
            one_label = 0
            for i in range(len(line_cov)):
                if (line_cov[i] == '1'):
                    one_label = one_label + 1
                    if one_label == item:
                        result_source.append(line_num[i])
#                        result_source.append(i)
...
def main():
    ...
    model.to(args.device)
    test_IA(args, model, tokenizer, best_threshold=0.5)
